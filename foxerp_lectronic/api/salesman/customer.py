...
def updateCustomer(data, customer_id=None):
    ...
    try:
        ...
        if data.get("customer_name_in_arabic"):
            customer.customer_name_in_arabic = data.get("customer_name_in_arabic")
...
def update_customer_address(data, address_id=None):
    ...
    if data.get('address_in_arabic'):
        addr.address_in_arabic = data.get('address_in_arabic')
...
@frappe.whitelist()
def updateCreditDetails(customer_id, credit_data=None, contact_person=None, credit_terms=None, credit_id=None):
    try:
        if credit_data:
    ...
    try:
        ...
        if frappe.request.data:
            ...
            if data.get("customer_name"):
                filters["customer_name"] = ('like', '%' + data.get("customer_name") + '%')
            if data.get("shop_name"):
                filters["shop_name"] = ['like', data.get("shop_name")]
            if data.get("phone_no"):
                filters["mobile_no"] = data.get("phone_no")
            if data.get("registration_date"):
                filters["registration_date"] = ['like', data.get("registration_date")]
        data = frappe.db.get_list('Customer',
                filters= filters,
                fields=['name as customer_id', 'customer_name','customer_name_in_arabic', 'disabled as status',
        ...
        for customer in data:
            ...
            customer["customer_name_in_arabic"] = customer["customer_name_in_arabic"]  or ""
...
def getCustomerDetails(customer_id):
    try:
        data = frappe.db.get_list('Customer',
            ...
            fields=['name as customer_id', 'customer_name', 'customer_name_in_arabic',
        ...
        for i in data:
            i["customer_name_in_arabic"] = i["customer_name_in_arabic"] or ""
            if i["status"] == 1:
                i["status"] = "Inactive"
            else:
                i["status"] = "Active" 
            ...
            if i["customer_primary_address"]:    
                ...
                i['address'] = frappe.get_value("Address", i["customer_primary_address"], 'address_line1')
                i['address_in_arabic'] = frappe.get_value("Address", i["customer_primary_address"], 'address_in_arabic') or ""
...
@frappe.whitelist()
def getPaymentTermsList():
    return frappe.db.get_list('Payment Term', pluck='name')
...
def getCustomerAccountsInfo():
    ...
    for sales_order in sales_order_list:
        ...
        individual_record["Customer Name In Arabic"] = sales_record.customer_name_in_arabic or ""
        ...
        sales_invoice_id = frappe.db.get_value('Sales Invoice Item', {'sales_order': sales_order},'parent')
        individual_record["Sales Invoice ID"] = sales_invoice_id or ""
        delivery_note = frappe.db.get_value('Delivery Note Item', {'against_sales_order': sales_order},
			['parent', 'warehouse'], as_dict=1)
        individual_record["Delivery Note ID"] = individual_record["Warehouse"] = ""
        if delivery_note:
            individual_record["Delivery Note ID"] = delivery_note.parent
            individual_record["Warehouse"] = delivery_note.warehouse
        ...
        accounts_record_list.append(individual_record)
    return_data["Account Records"] = accounts_record_list
...
def getCreditDetails(customer_id):
    ...
    try:
        ...
        data["customer_name_in_arabic"] = customer.get("customer_name_in_arabic") or ""
...
@frappe.whitelist()
def uploadCustomerFile(customer_id, other_document_name=None):
    """Upload the Customer related documents"""
    try:
        if customer_id:
            customer = frappe.get_doc("Customer",customer_id)
            file_field = list(frappe.request.files.to_dict().keys())[0]
            file_url = ""
            link_doctype = {
                "is_private":1,
                "doctype": "Customer",
            ...
            if frappe.request.files[file_field]:
                file_details = upload_attachment(frappe.request.files[file_field],link_doctype)
                file_url = file_details.get("file_url")
                setattr(customer, file_field, file_url)
                if file_field == "others":
                    customer.other_document_name = other_document_name
            if file_url and file_details.get("attached_to_name") == customer_id:
                customer.save(ignore_permissions=True)
                return {"file_url": file_url}
            else:
                frappe.throw(_("File not uploaded, please try again"))    
    except Exception:
        error = frappe.get_traceback()
        createAPIErrorLog(error)
        return error    