...
def execute(filters=None):
	...
	for item in data:
		...
		sales_invoice_data = frappe.db.sql("""
			...
			order by modified desc ;""".format(item.item_code, conditions), filters, as_dict=1)
		...
		for sales_data in sales_invoice_data:
			...
			if sales_data.delivery_note:
				unit_cost, total_cost = get_valuation_rate(sales_data.get("item_code"), sales_data, unit_cost, total_cost)
		...
		item["total_cost"] = sum_total_cost
		item["profit"] = base_net_amount - sum_total_cost
		if sum_total_cost == 0.0 and base_net_amount == 0.0:
			temp.append(item)
	for i in temp:
		data.remove(i)
	return columns, data
