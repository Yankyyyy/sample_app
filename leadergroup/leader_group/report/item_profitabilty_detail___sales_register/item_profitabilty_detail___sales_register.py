...
def execute_one(filters=None, additional_table_columns=None, additional_query_columns=None):
	...
	for d in item_list:
		...
		if d.delivery_note:
			unit_cost, total_cost = get_valuation_rate(d.item_code, d, unit_cost, total_cost)
...
def get_valuation_rate(item_code, row, unit_cost, total_cost):
	packed_item = frappe.db.sql("""select
			item_code, qty
		from
			`tabPacked Item`
		where
			...
			and parent = '{0}'
			and parent_item = '{1}'
		order by modified desc""".format(row.get('delivery_note'), item_code), as_dict=1)

	if packed_item:
		for item in packed_item:
			unit_cost += get_item_valuation_rate(item.get("item_code"), row)
			total_cost += get_item_valuation_rate(item.get("item_code"), row) * item.get("qty")
	else:
		unit_cost = get_item_valuation_rate(item_code, row)
		total_cost = get_item_valuation_rate(item_code, row) * row.get("qty")
...
def get_item_valuation_rate(item_code, row):
	...
	if row.get('delivery_note'):
		condition += " AND sle.voucher_no=%s" % (frappe.db.escape(row.delivery_note))
