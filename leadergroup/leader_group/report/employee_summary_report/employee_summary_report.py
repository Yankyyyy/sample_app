...
def get_data(filters):
	...
	for employee in employee_list:
		...
		# opening
		opening_debit, opening_credit = opening_balances.get(employee.name, [0, 0])
		row.update({
		...
		# within period
		debit, credit = balances_within_period.get(employee.name, [0, 0])
		row.update({
			"debit": debit,
			"credit": credit
		})
...
def get_opening_balances(filters):
	gle = frappe.db.sql("""
		...
		group by employee""",filters, as_dict=True)
