...
def execute(filters=None):
    ...
    if filters.get("account_list"):
        ...
        # get account list with account name 'input rcm'
        account_list = get_accounts(filters, None, "input rcm")
    ...
    if account_list:
        for account in account_list:
            ...
            net_total_amount= 0
            is_return = 0
            additional_columns = "I.posting_date, I.return_against, I.bill_no, I.supplier, I.tax_id"
            if filters.get("invoice_type") == "RCM Adjustment":
                is_return = 1
            
            # get invoice which having 'input rcm' charges in taxes and charges table
            invoice_data = get_tax_invoice_details(filters, "purchase", account, is_return, additional_columns)
            if invoice_data:
                data.append(
                    {
